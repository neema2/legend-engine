...
public class LateralJoin extends Shared
{
    ...
    public CoreInstance execute(ListIterable<? extends CoreInstance> params, Stack<MutableMap<String, CoreInstance>> resolvedTypeParameters, Stack<MutableMap<String, CoreInstance>> resolvedMultiplicityParameters, VariableContext variableContext, MutableStack<CoreInstance> functionExpressionCallStack, Profiler profiler, InstantiationContext instantiationContext, ExecutionSupport executionSupport, Context context, ProcessorSupport processorSupport) throws PureExecutionException
    {
        ...
        CoreInstance joinFunction = Instance.getValueForMetaPropertyToOneResolved(params.get(3), M3Properties.values, processorSupport);
        LambdaFunction<?> lambdaJoinFunction = (LambdaFunction<?>) LambdaFunctionCoreInstanceWrapper.toLambdaFunction(joinFunction);
        ...
        for (int i = 0; i < tds1.getRowCount(); i++)
        {
            ...
            TestTDS res = filter(exploded, joinFunction, resolvedTypeParameters, resolvedMultiplicityParameters, variableContext, functionExpressionCallStack, profiler, instantiationContext, executionSupport, processorSupport, relationtype);
            
            // Handle join type (LEFT or INNER)
            CoreInstance joinKind = Instance.getValueForMetaPropertyToOneResolved(params.get(2), M3Properties.values, processorSupport);
            String joinKindName = joinKind.getValueForMetaPropertyToOne("name").getName();
            
            if (res.getRowCount() == 0 && "LEFT".equals(joinKindName))
            {
    ...
    private TestTDS filter(TestTDS tds, CoreInstance filterFunction, Stack<MutableMap<String, CoreInstance>> resolvedTypeParameters, Stack<MutableMap<String, CoreInstance>> resolvedMultiplicityParameters, VariableContext variableContext, MutableStack<CoreInstance> functionExpressionCallStack, Profiler profiler, InstantiationContext instantiationContext, ExecutionSupport executionSupport, ProcessorSupport processorSupport, RelationType<?> relationtype)
    {
        ...
        {
            CoreInstance row = ValueSpecificationBootstrap.wrapValueSpecification(new TDSWithCursorCoreInstance(tds, i, "", null, relationtype, -1, repository, false), true, processorSupport);
            CoreInstance subResult = this.functionExecution.executeFunction(false, lambdaFunction, org.eclipse.collections.impl.factory.Lists.fixedSize.with(row, row), resolvedTypeParameters, resolvedMultiplicityParameters, evalVarContext, functionExpressionCallStack, profiler, instantiationContext, executionSupport);
